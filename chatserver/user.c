...
void view_online_users(OnlineUser* onlineUsers, int onlineUserCount, client_t* client) {
    ...
    for (int i = 0; i < onlineUserCount; i++) {
        if (strcmp(onlineUsers[i].username, client -> name) != 0) {
...
void remove_room_from_user(client_t* client, const char* room_name) {
    for (int i = 0; i < client -> room_count; i++) {
        if (strcmp(client -> room_names[i], room_name) == 0) {
            for (int j = i; j < client -> room_count - 1; j++) {
                strncpy(client -> room_names[j], client -> room_names[j + 1], sizeof(client -> room_names[j]));
            }
            client -> room_count--;
            break;
        }
