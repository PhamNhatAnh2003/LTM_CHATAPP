...
void handle_remove_member(chatroom_t* chatroom_list, client_t* user_list, Packet* packet, client_t* cli) {
    ...
    if (room &&target_client) {
        ...
        remove_room_from_user(target_client, room->room_name);
...
void remove_member_from_chatroom(chatroom_t* room, client_t* client, client_t* requester, char* response) {
    ...
    if (room == NULL) {
        ...
        send(requester->sockfd, res, sizeof(res), 0);
    } else {
        int index = -1;
        for (int i = 0; i < room->member_count; i++) {
        ...
        if (index != -1) {
            ...
            send(client->sockfd, res, sizeof(res), 0);
        } else {
...
void handle_leave_chatroom(chatroom_t* chatroom_list, Packet* packet, client_t* cli) {
    ...
    if (room) {
        ...
        remove_room_from_user(cli, room->room_name);
        send_message_to_chatroom(room, response, -1);
        packet->status = 1;
